...
use lotus_lib::package::{Package, PackageTrioType};
...
impl Texture for Package<CachePairReader> {
    ...
    fn decompress_texture(&self, node: &Node) -> Result<Vec<u8>> {
        let h_cache = self.get(&PackageTrioType::H);
        let f_cache = self.get(&PackageTrioType::F);
        let b_cache = self.get(&PackageTrioType::B);
